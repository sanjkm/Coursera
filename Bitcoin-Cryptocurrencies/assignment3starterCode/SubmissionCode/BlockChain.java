...
public class BlockChain {
	...
	// from the block
	// Add one to the size to account for the coinbase
	...
	// Get this UTXO pool, add the coinbase output to it, then
	// create a new TxHandler variable with the updated UTXO pool

	UTXOPool TestUTXOPool = NewChainBlockHandler.getUTXOPool();

	Transaction coinbase_tx = block.getCoinbase();
	UTXO coinbase_UTXO = new UTXO(coinbase_tx.getHash(), 0);
	TestUTXOPool.addUTXO (coinbase_UTXO, (coinbase_tx.getOutputs()).get(0));
	TxHandler FinalChainBlockHandler = new TxHandler (TestUTXOPool);
		
	MyTreeNode newNode = new MyTreeNode(new TreeChains (block, FinalChainBlockHandler, curr_age,
